...
| `connect` | 异步（`initFd` 设 NONBLOCK） | 异步（`initFd` 设 NONBLOCK） | 异步 | **ConnectEx** (IOCP) |
| `initFd()` | `fcntl(O_NONBLOCK)` | `fcntl(O_NONBLOCK)` | 不适用 | 不适用（overlapped） |
...
- [ ] `initFd()` 后 socket 已设为 NONBLOCK（kqueue/epoll），不会阻塞事件循环
- [ ] IOCP 后端 `ConnectEx` 前已 `bind()` socket（即使是通配地址）
- [ ] IOCP 后端提交阶段和完成阶段的错误映射已同步（all 4 I/O completion handlers）
- [ ] IOCP 后端的 cancel/async_wait/noop/close/shutdown 等非 IOCP 操作不会触发 stop_completion panic
- [ ] IOCP `close()` 使用原始 `kernel32.CloseHandle`，不用 Zig 标准库的 `windows.CloseHandle`
- [ ] kqueue `connect()` 路径能正确处理 `EISCONN`(56) 等连接 errno，不归入 `error.Unexpected`
...
## kqueue connect EISCONN (errno 56) 未映射导致崩溃

### 问题

macOS kqueue 后端在 `connect()` 一个已连接的 socket 时，POSIX `connect()` 返回 `EISCONN` (errno 56)，
但 libxev 的 `syscall_result` 的 `.connect` 分支未映射此错误，落入 `unexpectedErrno` → `error.Unexpected` → panic。

### 根因

`vendor/libxev/src/backend/kqueue.zig` 的 `syscall_result` 函数在 `.connect` 分支只处理了
少数几个 errno，缺少 `EISCONN`、`ETIMEDOUT`、`ENETUNREACH`、`EHOSTUNREACH`、`EADDRNOTAVAIL` 的映射。

### 修复（2025-07-22）

在 `syscall_result` 的 `.connect` 分支新增 5 个 errno 映射：

| errno | 映射结果 | 说明 |
|-------|---------|------|
| `EISCONN` (56) | 视为成功（空结果） | socket 已连接，等价于连接成功 |
| `ETIMEDOUT` | `error.ConnectionTimedOut` | 连接超时 |
| `ENETUNREACH` | `error.NetworkUnreachable` | 网络不可达 |
| `EHOSTUNREACH` | `error.HostUnreachable` | 主机不可达 |
| `EADDRNOTAVAIL` | `error.AddressNotAvailable` | 地址不可用 |

**关键**：`EISCONN` 在 macOS 上 `connect()` 已连接 socket 时返回，应视为成功而非错误。
错误名对齐 `ConnectError` 类型定义（`Timeout`/`AddressUnavailable`/`HostUnreachable`/`NetworkUnreachable`）。

## TCP/UDP initFd 设置 NONBLOCK 防止阻塞事件循环

### 问题

kqueue/epoll 后端在调用 `initFd()` 初始化 socket 时未设置 `O_NONBLOCK`，导致：

1. 上游代码（如 `egress.Socket.initTcp()`）创建的 socket 默认是**阻塞模式**
2. kqueue 后端 `connect()` 在 `start_completion()` 阶段**同步**调用 `posix.system.connect()`
3. 阻塞 socket 的 `connect()` 可能需要数秒才能完成（等待 TCP 握手）
4. 在此期间整个事件循环被卡死 — `accept`、`read`、`write` 等其他事件永不触发

**症状**：连接建立阶段事件循环停止响应，所有 I/O 操作挂起直到 `connect()` 完成或超时。

### 修复（2025-07-22）

在 `src/watcher/tcp.zig` 和 `src/watcher/udp.zig` 的 `initFd()` 函数中，
通过 `fcntl(F_SETFL, O_NONBLOCK)` 设置非阻塞模式：

```zig
// tcp.zig / udp.zig — initFd() 中新增
_ = std.posix.system.fcntl(fd, std.posix.F.SETFL, @bitCast(@as(u32, std.posix.O.NONBLOCK)));
```

同时移除 kqueue 后端 `connect()` 调用处的条件编译 `NONBLOCK` 注释，
因为现在所有通过 `initFd()` 创建的 socket 在进入事件循环前已经是非阻塞模式。

**影响范围**：kqueue (macOS) 和 epoll (Linux) 后端。io_uring 和 IOCP 后端不受影响
（io_uring 的 fd 注册时机不同，IOCP 使用 overlapped I/O 模型）。

## IOCP ConnectEx 异步连接

### 问题

IOCP 后端使用标准 `connect()` 在 overlapped socket 上执行连接。对于远程地址，
标准 `connect()` 返回 `WSAEWOULDBLOCK` 但**不会**向 IOCP 端口投递完成通知，
导致异步连接无法完成 —— 回调永远不会被触发。

### 根因

Windows overlapped socket 的异步连接需要使用 `ConnectEx` 扩展函数
（如同 accept 必须使用 `AcceptEx`），而 `connect()` 并非 IOCP 感知的 API。

### 修复（2025-07-26）

采用与 `AcceptEx` 相同的模式实现 `ConnectEx`：

**`src/windows.zig`** — 加载扩展函数：
```zig
const WSAID_CONNECTEX = Guid.parse("{25a207b9-ddf3-4660-8ee9-76e58c74063e}");
const LPFN_CONNECTEX = *const fn (...) callconv(.winapi) BOOL;
pub fn loadConnectEx(socket: SOCKET) !LPFN_CONNECTEX;
```

**`src/backend/iocp.zig`** — 5 处变更：

| 阶段 | 变更 |
|------|------|
| `handle()` | 返回 socket 用于 IOCP 关联（与 accept 一致） |
| `start_completion()` | `bind()` + `ConnectEx()` 替代同步 `connect()` |
| `perform()` | `WSAGetOverlappedResult()` + `SO_UPDATE_CONNECT_CONTEXT` 确认连接 |
| `stop_completion()` | `CancelIoEx()` 取消进行中的 ConnectEx |
| 错误处理 | ConnectEx 完成后的 WSA 错误映射 |
...
事件循环关闭时，`stop_completion()` 遍历所有活跃 Completion 并取消对应的 IOCP 操作。
但对于非 IOCP 操作类型（`.cancel`、`.async_wait`、`.noop`、`.close`、`.shutdown`、`.job_object`），
它们没有 IOCP overlapped 操作可取消，原代码的 `else` 分支直接 `panic("Not implemented")`。

**症状**：Windows 上 defer 清理使用 cancel completion 时，程序崩溃。

### 修复（2025-07-26）

将 `else` panic 分支改为**优雅降级**：直接将 Completion 标记为 dead 并递减活跃计数：

```zig
// iocp.zig — stop_completion() switch 语句
else => {
    // cancel, async_wait, noop, close, shutdown, job_object:
    // 没有 IOCP overlapped 操作需要取消
    c.flags.state = .dead;
    self.active -= 1;
},
```
...
IOCP 的**提交阶段**（`start_completion`）映射了 12 种 WSA 错误码，
但**完成阶段**（`perform` 的 completion handler）仅映射了 `Canceled` 和 `ConnectionResetByPeer` 2 种。
...
为全部 4 个 I/O 完成处理函数补充缺失的错误映射，使其与提交阶段一致：

| 完成处理 | 新增映射 |
|---------|---------|
| `send` | `+AddressNotAvailable`、`+NetworkUnreachable`、`+SocketNotConnected` |
| `recv` | `+AddressNotAvailable`、`+NetworkSubsystemFailed`、`+NetworkUnreachable`、`+SocketNotConnected` |
| `sendto` | `+AddressNotAvailable`、`+NetworkUnreachable`、`+SocketNotConnected` |
| `recvfrom` | `+AddressNotAvailable`、`+NetworkSubsystemFailed`、`+NetworkUnreachable`、`+SocketNotConnected` |

同时在 `unexpectedWSAError()` 中增加诊断日志，帮助识别后续可能的未映射错误码。

### 两阶段错误映射模型

此修复揭示了一个重要的 IOCP 行为差异：

| 阶段 | 发生的错误类型 | 示例 |
|------|-------------|------|
| **提交阶段** | 参数校验、状态错误 | `WSAENOTSOCK`、`WSAEWOULDBLOCK`、`WSAENOTCONN` |
| **完成阶段** | 网络层异步错误 | `WSAEHOSTUNREACH`、`WSAENETUNREACH`、`WSAECONNRESET` |

**两个阶段的错误映射必须保持同步**，否则网络层异步错误会在完成阶段被吞没为 `error.Unexpected`。

## IOCP CloseHandle STATUS_INVALID_HANDLE 修复

### 问题

IOCP 后端在 `close()` 操作中使用 Zig 标准库的 `windows.CloseHandle()` 关闭 socket，
但该函数对无效句柄（-1）会触发 `STATUS_INVALID_HANDLE` 异常而非返回错误。

### 修复（2025-07-18）

改用原始 `kernel32.CloseHandle()` 直接调用，绕过 Zig 标准库的句柄有效性检查：

```zig
// ✅ 正确：绕过 Zig 标准库的句柄检查
const CloseHandle = kernel32.CloseHandle;
_ = CloseHandle(socket);
```

类似地，`closesocket()` 也使用直接系统调用避免中间层的异常行为。

## kqueue Timer 时钟更新修复

### 问题

kqueue 后端 `timer_next()` 计算下次超时时间时使用 `self.now`，
但长时间运行后 `self.now` 可能未及时更新，导致定时器精度下降。

### 修复（2025-07-18）

在 `timer_next()` 开头调用 `update_now()` 刷新时钟：

```zig
fn timer_next(self: *Loop) i64 {
    @branchHint(.cold);
    self.update_now();  // ← 确保使用最新时钟
    var min_ms: ?i64 = null;
    // ... 遍历 timer 列表 ...
}
```
